...
const blankFull = new Jimp(72, 72, 0x00000000)
const blankBase64 = blankFull.getBase64Async(Jimp.MIME_PNG)
...
class CaptivateInstance extends InstanceBase {
	...
	async _queryFeedbackState(fullId, options) {
		let logThis = false
		if (!fullId == 'newblue.core.layers~playout.feedback.native.layerPreview') {
			logThis = true
		}
		...
		try {
			...
			if (logThis) {
				console.log('_cmp_v1_queryFeedbackState response', { actorId, feedbackId, options, reply })
			}
			if (!reply) {
				return {}
			}
...
module.exports = CaptivateInstance
