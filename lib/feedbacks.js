...
module.exports = {
  ...
  publishFeedbacks(response) {
    this.debug("publishing feedbacks");
    ...
    if (response != undefined) {
      ...
      response.forEach((feedback) => {
        ...
        // Older controllers use `feedback.native` or `feedback.matcher` in their feedbacks
        // expect the 'advanced' type.
        // Newer controllers may also specify the `boolean` feedback type.
        feedback.type = feedback.id.match(/feedback\.boolean/) ? 'boolean' : 'advanced';
        ...
        feedback.subscribe = (feedback) => {
          ...
          if (!this.feedbackInstances.has(feedback.feedbackId)) {this.feedbackInstances.set(feedback.feedbackId, []);}
        ...
        feedback.callback = async (event) => {
          ...
          return this.handleFeedback(event); // will look in the cache for data
        };
